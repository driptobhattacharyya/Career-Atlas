...
## Corpus Check
- 151 files · ~200,456 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 776 nodes · 1137 edges · 60 communities detected
- Extraction: 65% EXTRACTED · 35% INFERRED · 0% AMBIGUOUS · INFERRED: 403 edges (avg confidence: 0.66)
...
8. `resolve_role_slug()` - 12 edges
9. `job_finder_agent()` - 12 edges
10. `JobResult` - 12 edges
...
- `GitHubRepoInfo` --uses--> `Manifest-first ordering: manifests, then README, then entrypoints, then     sha`  [INFERRED]
  backend\app\github_analysis\schemas.py → backend\app\github_analysis\service.py
- `GitHubRepoInfo` --uses--> `CATRK-13: any skill whose evidence doesn't quote a fetched path or a real     l`  [INFERRED]
  backend\app\github_analysis\schemas.py → backend\app\github_analysis\service.py
- `test_build_job_result_maps_scores_and_explanation()` --calls--> `_build_job_result()`  [INFERRED]
  backend\tests\test_job_hunter_agent.py → backend\app\job_hunter\agent.py
...
Cohesion: 0.08
Nodes (53): node_judge(), Deep Researcher Agent — LangGraph topology.    plan -> search -> [critic router], evaluate_pathway(), _gaps_text(), Deep Researcher — LLM-as-judge evaluation.  Scores a generated Pathway against a, Run the rubric judge over a pathway. Returns a structured verdict., Run the rubric judge over a pathway. Returns a structured verdict., _validation_text() (+45 more)
...
Cohesion: 0.06
Nodes (44): latest_resume_id(), Shared resume-lookup helpers.  Several routers need "the user's resumes, newes, All of the user's resume ids, newest first. Strictly scoped to user_id., The user's most recent resume id, or None if they have none., user_resume_ids(), add_skill(), _delete_other_resumes(), delete_skill() (+36 more)

### Community 5 - "Community 5"
Cohesion: 0.1
Nodes (34): BaseModel, confirm_github_skills(), get_github_insights(), Powers the insights panel: stored profile + per-repo facts + quarantined     sk, Powers the insights panel: stored profile + per-repo facts + quarantined     sk, Powers the insights panel: stored profile + per-repo facts + quarantined     sk, Promote suggested skills into the verified profile (confirmed=true). Gap     an, Promote suggested skills into the verified profile (confirmed=true). Gap     an (+26 more)
...
Cohesion: 0.1
Nodes (30): fetch_authenticated_login(), fetch_commit_stats(), fetch_file_content(), fetch_github_graphql(), fetch_languages(), fetch_repo_file_tree(), The viewer's login — needed to filter commits to owner-authored only., GET /languages -> bytes per language, returned as rounded percentages. (+22 more)
...
Cohesion: 0.24
Nodes (8): _decode(), _get_jwks(), Auth dependency — verifies Supabase user JWTs.  The Supabase project signs token, Decode + verify a token against a JWKS dict; return the user id (sub)., Validate the Supabase JWT from the Authorization header.      Returns the user U, Like `get_current_user_id`, but rejects unauthenticated requests.      Routes, require_user_id(), verify_token()
...
- **Thin community `Community 116`** (1 nodes): `Returns a configured HuggingFace model instance. Good for auxiliary subtasks.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 117`** (1 nodes): `Guarantee taxonomy rows exist for this role.      Returns the taxonomy source:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 118`** (1 nodes): `Returns a configured Groq model instance. Best for fast, responsive generation.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 119`** (1 nodes): `Returns a configured HuggingFace model instance. Good for auxiliary subtasks.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 120`** (1 nodes): `Replace-on-upload: keep one resume per user.      All child tables (skills, ex`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 121`** (1 nodes): `The user's most recent resume id, or None if they have none.      Strictly sco`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 122`** (1 nodes): `Change the user's target role (persisted in the `profiles` table).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 123`** (1 nodes): `Add a skill to the user's latest resume.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 124`** (1 nodes): `Remove a skill (by name) from the user's latest resume.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 125`** (1 nodes): `Returns a configured Gemini model instance. Best for complex reasoning and large`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 126`** (1 nodes): `Returns a configured Groq model instance. Best for fast, responsive generation.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 127`** (1 nodes): `Returns a ChatOpenAI instance pointed at OpenRouter.     Any model slug from op`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 128`** (1 nodes): `Returns a configured HuggingFace model instance. Good for auxiliary subtasks.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 129`** (1 nodes): `Upload a resume file to the private bucket; return its object path.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 130`** (1 nodes): `Download a resume file by its storage object path.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 131`** (1 nodes): `Intentionally errors — used to verify Sentry capture end-to-end.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 132`** (1 nodes): `LLM structured output schema.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 133`** (1 nodes): `Full API response returned to the frontend.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 134`** (1 nodes): `Replace-on-upload: keep one resume per user.      All child tables (skills, ex`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 135`** (1 nodes): `Change the user's target role (persisted in the `profiles` table).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 136`** (1 nodes): `Add a skill to the user's latest resume.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 137`** (1 nodes): `Remove a skill (by name) from the user's latest resume.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 138`** (1 nodes): `Edit an experience entry's title / company / dates.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 139`** (1 nodes): `LLM structured output schema.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 140`** (1 nodes): `Full API response returned to the frontend.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 141`** (1 nodes): `Coerce the LLM's free-text 'posted' value into a day count for the int column.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 142`** (1 nodes): `Return the current user's persisted job matches (latest search).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 143`** (1 nodes): `Replace-on-upload: keep one resume per user.      All child tables (skills, ex`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 144`** (1 nodes): `Returns a configured Groq model instance. Best for fast, responsive generation.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 145`** (1 nodes): `Returns a ChatOpenAI instance pointed at OpenRouter.     Any model slug from op`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 146`** (1 nodes): `Returns a configured HuggingFace model instance. Good for auxiliary subtasks.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 147`** (1 nodes): `Validates the JWT token coming from the frontend (which signed in via InsForge).`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 148`** (1 nodes): `Converts a display title like "Machine Learning Engineer"     into its Pinecone`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 149`** (1 nodes): `Builds a BM25 index from Pinecone taxonomy metadata for a specific role.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 150`** (1 nodes): `Full hybrid retrieval pipeline:       1. Semantic search in Pinecone with role`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 151`** (1 nodes): `Returns a configured Tavily search tool for LangGraph agent.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 152`** (1 nodes): `Returns a configured Gemini model instance. Best for complex reasoning and large`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 153`** (1 nodes): `Returns a configured Groq model instance. Best for fast, responsive generation.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 154`** (1 nodes): `Returns a configured HuggingFace model instance. Good for auxiliary subtasks.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 155`** (1 nodes): `Saves bytes to a temporary file and extracts clean markdown using pymupdf4llm.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 156`** (1 nodes): `Feeds markdown to Gemini with strict structured output.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `ResumeExtraction` connect `Community 4` to `Community 1`, `Community 5`?**
  _High betweenness centrality (0.052) - this node is a cross-community bridge._
- **Why does `get_groq_model()` connect `Community 10` to `Community 0`, `Community 9`, `Community 2`, `Community 1`?**
  _High betweenness centrality (0.049) - this node is a cross-community bridge._
- **Why does `_invoke_llm()` connect `Community 1` to `Community 10`?**
  _High betweenness centrality (0.038) - this node is a cross-community bridge._
- **Are the 21 inferred relationships involving `ResumeExtraction` (e.g. with `ProfileUpdate` and `SkillIn`) actually correct?**
  _`ResumeExtraction` has 21 INFERRED edges - model-reasoned connections that need verification._
