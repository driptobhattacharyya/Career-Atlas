...
async def research_jobs(req: ResearchJobsRequest, user_id: str = Depends(get_current_user_id)):
    try:
        ...
        try:
            ...
            # Keep the lookup scoped to this user even on the retry path — a
            # global fallback could hand one user another user's resume.
            logger.warning("resume lookup failed for user %s; retrying scoped", user_id, exc_info=True)
            resume_resp = (
                db_client.table("resumes")
                .select("id")
                .eq("user_id", user_id)
