...
async def fetch_full_resume(resume_id: str) -> dict[str, Any]:
    # Optimization: Eliminate N+1 queries and parallelize DB lookups.
    # 1. Fetch all top-level resume relationships concurrently
    results = await asyncio.gather(
        _execute_async(db_client.table("resumes").select("*").eq("id", resume_id)),
        _execute_async(db_client.table("contacts").select("*").eq("resume_id", resume_id)),
        _execute_async(db_client.table("skills").select("*").eq("resume_id", resume_id)),
        _execute_async(db_client.table("programming_languages").select("*").eq("resume_id", resume_id)),
        _execute_async(db_client.table("spoken_languages").select("*").eq("resume_id", resume_id)),
        _execute_async(db_client.table("keywords").select("*").eq("resume_id", resume_id)),
        _execute_async(db_client.table("experiences").select("*").eq("resume_id", resume_id)),
        _execute_async(db_client.table("education").select("*").eq("resume_id", resume_id)),
        _execute_async(db_client.table("projects").select("*").eq("resume_id", resume_id)),
        _execute_async(db_client.table("certifications").select("*").eq("resume_id", resume_id)),
        return_exceptions=True
    )

    resume_res = results[0].data if not isinstance(results[0], Exception) else []
    resume = resume_res[0] if resume_res else {}

    contact_rows = results[1].data if not isinstance(results[1], Exception) else []
    contact = contact_rows[0] if contact_rows else {}

    skills = [x["skill"] for x in (results[2].data if not isinstance(results[2], Exception) else [])]
    prog_langs = [x["language"] for x in (results[3].data if not isinstance(results[3], Exception) else [])]
    spoken_langs = [x["language"] for x in (results[4].data if not isinstance(results[4], Exception) else [])]
    keywords = [x["keyword"] for x in (results[5].data if not isinstance(results[5], Exception) else [])]

    exp_rows = results[6].data if not isinstance(results[6], Exception) else []
    edu_rows = results[7].data if not isinstance(results[7], Exception) else []
    proj_rows = results[8].data if not isinstance(results[8], Exception) else []
    certifications = results[9].data if not isinstance(results[9], Exception) else []

    exp_ids = [x["id"] for x in exp_rows]
    edu_ids = [x["id"] for x in edu_rows]
    proj_ids = [x["id"] for x in proj_rows]

    # 2. Batch fetch child records for experiences, education, and projects concurrently using .in_()
    child_queries = []
    if exp_ids:
        child_queries.extend([
            _execute_async(db_client.table("experience_bullets").select("*").in_("experience_id", exp_ids)),
            _execute_async(db_client.table("experience_technologies").select("*").in_("experience_id", exp_ids))
        ])
    else:
        child_queries.extend([asyncio.sleep(0), asyncio.sleep(0)]) # Dummy for consistent indexing

    if edu_ids:
        child_queries.append(_execute_async(db_client.table("education_notes").select("*").in_("education_id", edu_ids)))
    else:
        child_queries.append(asyncio.sleep(0))

    if proj_ids:
        child_queries.append(_execute_async(db_client.table("project_technologies").select("*").in_("project_id", proj_ids)))
    else:
        child_queries.append(asyncio.sleep(0))

    child_results = await asyncio.gather(*child_queries, return_exceptions=True)

    # 3. Stitch children back onto parents in memory
    exp_bullets_map = defaultdict(list)
    if exp_ids and not isinstance(child_results[0], Exception) and child_results[0]:
        for row in child_results[0].data:
            exp_bullets_map[row["experience_id"]].append(row["bullet"])

    exp_techs_map = defaultdict(list)
    if exp_ids and not isinstance(child_results[1], Exception) and child_results[1]:
        for row in child_results[1].data:
            exp_techs_map[row["experience_id"]].append(row["tech"])

    edu_notes_map = defaultdict(list)
    if edu_ids and not isinstance(child_results[2], Exception) and child_results[2]:
        for row in child_results[2].data:
            edu_notes_map[row["education_id"]].append(row["note"])

    proj_techs_map = defaultdict(list)
    if proj_ids and not isinstance(child_results[3], Exception) and child_results[3]:
        for row in child_results[3].data:
            proj_techs_map[row["project_id"]].append(row["tech"])

    experiences = []
    for exp in exp_rows:
        exp["description_bullets"] = exp_bullets_map[exp["id"]]
        exp["technologies"] = exp_techs_map[exp["id"]]
        experiences.append(exp)

    education = []
    for edu in edu_rows:
        edu["notes"] = edu_notes_map[edu["id"]]
        education.append(edu)

    projects = []
    for proj in proj_rows:
        proj["technologies"] = proj_techs_map[proj["id"]]
