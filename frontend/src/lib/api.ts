...
// Simple apiClient for custom requests (e.g., used by GitHub analysis)
export const apiClient = {
  get: async (path: string) => request(path, { method: "GET" }).then(data => ({ data })),
  post: async (path: string, body?: any) => request(path, { method: "POST", body: body ? JSON.stringify(body) : undefined, headers: { "Content-Type": "application/json" } }).then(data => ({ data }))
};
