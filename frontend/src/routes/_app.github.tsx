...
function GithubConnectPage() {
  ...
  const { data: statusData, isLoading: statusLoading } = useQuery({
    ...
    queryFn: () => request<any>("/api/github/status"),
  });

  // 2. If connected, fetch repos
  const { data: reposData, isLoading: reposLoading } = useQuery({
    queryKey: ["github-repos"],
    queryFn: () => request<any>("/api/github/repos"),
  ...
  const analyzeMutation = useMutation({
    mutationFn: (repos: string[]) =>
      request<any>("/api/github/analyze", {
        method: "POST",
      ...
      <div className="container max-w-2xl py-12">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Connect your GitHub account to analyze your repositories and technical capabilities.
            </CardDescription>
          </CardHeader>
          <CardContent>
