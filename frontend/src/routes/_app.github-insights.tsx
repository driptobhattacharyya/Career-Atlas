    ...
    return (
      <div className="container max-w-2xl py-12 text-center space-y-4">
        ...
        <p className="text-muted-foreground">
          You haven't analyzed any repositories yet. Connect GitHub and we'll match your code to
          skills.
        </p>
    ...
    <div className="container max-w-3xl py-12 space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight flex items-center gap-2">
          <Github className="w-7 h-7" /> GitHub Insights
        </h1>
        <p className="text-muted-foreground mt-2">
          We matched your repositories to skills. Strong matches were added to your profile
          automatically — review the rest below. Only skills you keep are shown to employers.
        </p>
      </div>
      ...
      {profile && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              How you tend to build, based on the code in your repositories.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3 text-sm">
            <p>{profile.analysis_summary}</p>
            {profile.coding_behavior && (
              <p className="text-muted-foreground">{profile.coding_behavior}</p>
            )}
          </CardContent>
        </Card>
      )}
          ...
          <CardHeader>
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <CardDescription>
                  We're less sure about these. Add the ones that fit.
                </CardDescription>
              </div>
              <Button
                size="sm"
                ...
                <div key={e.id} className="flex items-start justify-between gap-4 p-4">
                  <div className="flex-1 space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full ${confColor[e.confidence || "low"]}`}
                      >
                    ...
                    {e.evidence && (
                      <p className="text-xs text-muted-foreground">
                        Where we saw it: {e.evidence}
                        {e.source_repo ? ` (${shortRepo(e.source_repo)})` : ""}
                      </p>
                    )}
                  </div>
      ...
      {added.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Strong matches from your code. Remove any that don't belong.
            </CardDescription>
          </CardHeader>
          <CardContent className="p-0">
            <div className="divide-y">
            ...
            <Card key={repo.repo_name}>
              ...
              <CardContent className="space-y-2 text-sm">
                ...
                {repo.languages && Object.keys(repo.languages).length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {Object.entries(repo.languages)
                      .sort((a, b) => b[1] - a[1])
                      .map(([lang, pctVal]) => (
                        <span key={lang} className="text-xs bg-muted px-2 py-0.5 rounded-full">
                          {lang} {pctVal}%
                        </span>
                      ))}
                  </div>
                ...
                {repo.commit_count != null && repo.commit_count > 0 && (
                  <p className="text-xs text-muted-foreground">
                    {repo.commit_count}
                    {repo.commit_count >= 100 ? "+" : ""} commits by you
